...
const EditQuotationPage: React.FC = () => {
  ...
  // متغيرات الحالة للتعديل
  const [originalQuotation, setOriginalQuotation] = useState<QuotationData & { 
    items?: Array<{ itemCode: string; itemName: string; quantity: number; unit: string; price: number; discountPercent: number; taxPercent: number; }>; 
    customer?: { id: string; name: string; commercialReg?: string; taxNumber?: string; };
    statement?: string;
    movementType?: string;
    accountType?: string;
  } | null>(null);
  ...
  const [quotationDate, setQuotationDate] = useState<dayjs.Dayjs>(dayjs());
  ...
  // دالة جلب بيانات عرض السعر للتعديل
  const fetchQuotationData = useCallback(async () => {
    ...
    try {
      ...
      const quotationSnapshot = await getDoc(quotationDoc);
      
      if (!quotationSnapshot.exists()) {
        message.error('عرض السعر غير موجود');
        navigate('/stores/quotations');
        return;
      }

      const data = quotationSnapshot.data() as QuotationData & { 
        items?: Array<{ itemCode: string; itemName: string; quantity: number; unit: string; price: number; discountPercent: number; taxPercent: number; }>; 
        customer?: { id: string; name: string; commercialReg?: string; taxNumber?: string; };
        statement?: string;
        movementType?: string;
      ...
      setQuotationData({
        ...
        date: String(data.date || dayjs().format('YYYY-MM-DD')),
        branch: String(data.branch || ''),
        warehouse: String(data.warehouse || ''),
        customerNumber: String(data.customer?.id || data.customerNumber || ''),
        customerName: String(data.customer?.name || data.customerName || ''),
        delegate: String(data.delegate || ''),
        priceRule: String(data.priceRule || ''),
        commercialRecord: String(data.customer?.commercialReg || ''),
        taxFile: String(data.customer?.taxNumber || ''),
      ...
      message.error('حدث خطأ في جلب بيانات عرض السعر');
      navigate('/stores/quotations');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // إعداد بيانات عرض السعر للتحديث
      ...
      // تحديث في Firebase
      if (!quotationId) {
        throw new Error('معرف عرض السعر مطلوب للتحديث');
      }
      
      const quotationDoc = doc(db, 'quotations', quotationId);
      await updateDoc(quotationDoc, quotationToUpdate);
      
      message.success('تم تحديث عرض السعر بنجاح');
      
      // العودة إلى صفحة قائمة عروض الأسعار
      navigate('/stores/quotations');
  ...
  const generateAndSetQuotationNumber = useCallback(async (branchIdValue: string) => {
    const newQuotationNumber = await generateQuotationNumberAsync(branchIdValue, branches);
    setQuotationNumber(newQuotationNumber);
    setQuotationData(prev => ({ ...prev, quotationNumber: newQuotationNumber }));
  }, [branches, setQuotationNumber]);
  ...
  useEffect(() => {
    ...
    // في صفحة التعديل، لا نحتاج لتوليد رقم عرض سعر جديد
    // توليد رقم عرض سعر جديد عند تغيير الفرع (معطل في صفحة التعديل)
    // if (quotationData.branch && branches.length && !quotationId) {
    //   generateAndSetQuotationNumber(quotationData.branch);
    // }
  }, [quotationData.branch, warehouses, branches.length, quotationId]);

  // توليد رقم عرض سعر تلقائي عند تحميل البيانات أو تغيير الفرع (معطل في صفحة التعديل)
  useEffect(() => {
    if (branches.length && quotationData.branch && !quotationId) {
      generateAndSetQuotationNumber(quotationData.branch);
    }
  }, [branches, quotationData.branch, generateAndSetQuotationNumber, quotationId]);

  // تعيين أول فرع افتراضياً عند تحميل البيانات (معطل في صفحة التعديل)
  useEffect(() => {
    if (branches.length && !quotationData.branch && !quotationId) {
      const firstBranch = branches[0];
      setQuotationData(prev => ({ ...prev, branch: firstBranch.id }));
    }
  }, [branches, quotationData.branch, quotationId]);
  ...
  return (
    <div className="p-4 space-y-6 font-['Tajawal'] bg-gray-50 min-h-screen">
      {isLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <Spin size="large" />
            <div className="mt-4 text-center">جاري تحميل بيانات عرض السعر...</div>
          </div>
        </div>
      )}
      ...
      {/* عرض باقي المحتوى فقط إذا تم تحميل البيانات */}
      {!isLoading && (
        <>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex flex-col ">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-1">تعديل عرض سعر</h1>
          <p className="text-gray-600 dark:text-gray-400">تعديل بيانات عرض السعر الموجود</p>
        </div>
      </div>
