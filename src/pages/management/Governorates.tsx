...
import { SearchOutlined, PlusOutlined, EditOutlined, DeleteOutlined, PrinterOutlined, DownloadOutlined, EyeOutlined } from '@ant-design/icons';
...
const Governorates: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const filtered = governorates.filter(gov => 
      gov.name?.toLowerCase().includes(searchText.toLowerCase()) ||
      gov.nameAr?.toLowerCase().includes(searchText.toLowerCase()) ||
      gov.nameEn?.toLowerCase().includes(searchText.toLowerCase())
    );
    setFilteredGovernorates(filtered);
    setCurrentPage(1);
  }, [searchText, governorates]);

  // البيانات المقسمة على صفحات
  const paginatedData = useMemo(() => {
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    return filteredGovernorates.slice(startIndex, endIndex);
  }, [filteredGovernorates, currentPage, pageSize]);

  // فتح المودال للإضافة أو التعديل
  const handleOpenModal = (governorate?: Governorate) => {
    if (governorate) {
  ...
  const handleSave = async (values: FormValues) => {
    try {
      if (editingGovernorate) {
        // تعديل محافظة موجودة
        const docRef = doc(db, 'governorates', editingGovernorate.id);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      message.error('حدث خطأ في حذف المحافظة');
    }
  };
  ...
  const handleExport = async () => {
    ...
    const exportData = filteredGovernorates.map((gov, index) => ({
      number: index + 1,
      name: gov.name || gov.nameAr || '',
      nameAr: gov.nameAr || gov.name || '',
      nameEn: gov.nameEn || '',
      createdAt: gov.createdAt ? dayjs(gov.createdAt.toDate()).format('DD/MM/YYYY') : ''
    }));

    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('المحافظات');

    sheet.columns = [
      { header: '#', key: 'number', width: 10 },
      { header: 'اسم المحافظة', key: 'name', width: 30 },
    ...
    a.href = url;
    a.download = `المحافظات_${new Date().toLocaleDateString('ar-SA').replace(/\//g, '-')}.xlsx`;
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 45%;">اسم المحافظة</th>
              <th style="width: 45%;">تاريخ الإنشاء</th>
            </tr>
          </thead>
          <tbody>
            ${filteredGovernorates.map((gov, index) => `
              <tr>
                <td>${index + 1}</td>
                <td>${gov.name || gov.nameAr || ''}</td>
                <td>${gov.createdAt ? dayjs(gov.createdAt.toDate()).format('DD/MM/YYYY') : ''}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'name',
      align: 'center' as const,
      render: (text: string, record: Governorate) => (
    ...
    },
    {
      ...
      align: 'center' as const,
      width: 200,
      render: (_: unknown, record: Governorate) => (
        <Space size="small">
          <Button
            type="link"
            icon={<EditOutlined />}
          ...
          <Popconfirm
            title={<span style={{ fontSize: '15px' }}>هل أنت متأكد من حذف هذه المحافظة؟</span>}
  ...
  return (
    <div className="w-full p-4 sm:p-6 space-y-6 min-h-screen" dir="rtl">
      <Helmet>
        <title>إدارة المحافظات | ERP90 Dashboard</title>
        <meta name="description" content="إدارة المحافظات في نظام ERP90" />
      </Helmet>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-1">إدارة المحافظات</h1>
              <p className="text-gray-600 dark:text-gray-400">إدارة وعرض المحافظات في النظام</p>
            </div>
          </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <div>
            <label style={labelStyle}>البحث عن محافظة</label>
            <Input
      ...
      <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
        <Table
          ...
          pagination={{
            current: currentPage,
            pageSize: pageSize,
            total: filteredGovernorates.length,
            onChange: (page) => setCurrentPage(page),
            showSizeChanger: false,
            showTotal: (total) => `إجمالي ${total} محافظة`,
      ...
      {/* Modal for Add/Edit */}
      <Modal
        title={editingGovernorate ? 'تعديل محافظة' : 'إضافة محافظة جديدة'}
        open={isModalVisible}
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="أدخل اسم المحافظة" size="large" />
          </Form.Item>
          ...
          <Form.Item
            ...
            name="nameAr"
          >
            <Input placeholder="أدخل الاسم بالعربي" size="large" />
          </Form.Item>
          ...
          <Form.Item
            ...
            name="nameEn"
          >
            <Input placeholder="Enter name in English" size="large" />
          </Form.Item>
